...
class OtpServiceTest {
    ...
    @Test
    @DisplayName("resetPassword - new password same as current encrypted password returns failure")
    void resetPassword_samePassword_returnsFailure() {
        OtpEntry entry = buildValidOtpEntry("654321");
        User user = new User(1L, "Test", EMAIL, "oldHash", "MEMBER");
        ...
        when(otpRepo.findTopByEmailOrderByExpiresAtDesc(EMAIL))
            .thenReturn(Optional.of(entry));
        when(userRepo.findByEmail(EMAIL)).thenReturn(Optional.of(user));
        // Simulate BCrypt confirming the raw new password matches the stored hash
        when(encoder.matches("Gym@123", "oldHash")).thenReturn(true);

        ResetPasswordRequest req = new ResetPasswordRequest();
        req.setEmail(EMAIL);
        req.setOtp("654321");
        req.setNewPassword("Gym@123");

        ForgotPasswordResponse resp = otpService.resetPassword(req);

        assertFalse(resp.isSuccess());
        assertTrue(resp.getMessage().toLowerCase().contains("different"));
        // Password must never be re-encoded/saved when rejected
        verify(encoder, never()).encode(anyString());
        verify(userRepo, never()).save(any());
        // OTP must remain unused so the user can retry with a different password
        assertFalse(entry.isUsed());
        verify(otpRepo, never()).save(any(OtpEntry.class));
    }

    @Test
    @DisplayName("resetPassword - new password different from current encrypted password succeeds")
    void resetPassword_differentPassword_succeeds() {
        OtpEntry entry = buildValidOtpEntry("654321");
        User user = new User(1L, "Test", EMAIL, "oldHash", "MEMBER");
        ...
        when(otpRepo.findTopByEmailOrderByExpiresAtDesc(EMAIL))
            .thenReturn(Optional.of(entry));
        when(userRepo.findByEmail(EMAIL)).thenReturn(Optional.of(user));
        when(encoder.matches("NewGym@456", "oldHash")).thenReturn(false);
        when(encoder.encode("NewGym@456")).thenReturn("newHash");
        when(userRepo.save(any(User.class))).thenReturn(user);
        when(otpRepo.save(any(OtpEntry.class))).thenReturn(entry);
        ...
        req.setEmail(EMAIL);
        req.setOtp("654321");
        req.setNewPassword("NewGym@456");

        ForgotPasswordResponse resp = otpService.resetPassword(req);

        assertTrue(resp.isSuccess());
        assertEquals("newHash", user.getPassword());
        verify(encoder).matches("NewGym@456", "oldHash");
        verify(encoder).encode("NewGym@456");
        verify(userRepo).save(user);
        assertTrue(entry.isUsed());
    }
