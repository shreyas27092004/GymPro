...
class AuthServiceTest {
    ...
    // ================================================================
    // login() – TRAINER approval-gate tests
    // ================================================================

    @Test
    @DisplayName("login - TRAINER with ACTIVE status: succeeds and returns a token")
    void login_trainerActive_succeeds() {
        sampleUser.setRole("TRAINER");
        sampleUser.setPassword("hashedPassword");
        when(repo.findByEmail(loginRequest.getEmail())).thenReturn(Optional.of(sampleUser));
        when(encoder.matches("rawPassword", "hashedPassword")).thenReturn(true);

        Map<String, Object> trainerProfile = new HashMap<>();
        trainerProfile.put("status", "ACTIVE");
        when(trainerServiceClient.getByEmail(sampleUser.getEmail())).thenReturn(trainerProfile);
        when(jwtUtil.generateToken(sampleUser.getEmail(), sampleUser.getRole(), sampleUser.getId()))
                .thenReturn("mocked-jwt-token");
    ...
    @Test
    @DisplayName("login - TRAINER with PENDING status: blocked with TrainerNotApprovedException")
    void login_trainerPending_isBlocked() {
        sampleUser.setRole("TRAINER");
        sampleUser.setPassword("hashedPassword");
        when(repo.findByEmail(loginRequest.getEmail())).thenReturn(Optional.of(sampleUser));
        when(encoder.matches("rawPassword", "hashedPassword")).thenReturn(true);
        ...
        Map<String, Object> trainerProfile = new HashMap<>();
        trainerProfile.put("status", "PENDING");
        when(trainerServiceClient.getByEmail(sampleUser.getEmail())).thenReturn(trainerProfile);

        TrainerNotApprovedException ex = assertThrows(
                TrainerNotApprovedException.class,
                () -> authService.login(loginRequest)
        );
    ...
    @Test
    @DisplayName("login - TRAINER with REJECTED status: blocked with a rejection message")
    void login_trainerRejected_isBlocked() {
        sampleUser.setRole("TRAINER");
        sampleUser.setPassword("hashedPassword");
        when(repo.findByEmail(loginRequest.getEmail())).thenReturn(Optional.of(sampleUser));
        when(encoder.matches("rawPassword", "hashedPassword")).thenReturn(true);
        ...
        Map<String, Object> trainerProfile = new HashMap<>();
        trainerProfile.put("status", "REJECTED");
        when(trainerServiceClient.getByEmail(sampleUser.getEmail())).thenReturn(trainerProfile);
    ...
    @Test
    @DisplayName("login - TRAINER whose profile lookup fails: blocked (fails closed, not open)")
    void login_trainerLookupFails_isBlocked() {
        sampleUser.setRole("TRAINER");
        sampleUser.setPassword("hashedPassword");
        when(repo.findByEmail(loginRequest.getEmail())).thenReturn(Optional.of(sampleUser));
        when(encoder.matches("rawPassword", "hashedPassword")).thenReturn(true);
        when(trainerServiceClient.getByEmail(sampleUser.getEmail()))
                .thenThrow(new RuntimeException("trainer-service unreachable"));
