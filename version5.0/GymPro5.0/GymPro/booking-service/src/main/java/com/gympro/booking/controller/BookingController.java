...
public class BookingController {
    ...
    @Operation(
        ...
        description = """
                      Transitions the booking to COMPLETED status. TRAINER or ADMIN only.

                      Future-dated bookings require explicit confirmation:
                        - Call without confirm (or confirm=false) first. If the booking's
                          session date is in the future, the response has
                          confirmationRequired=true, the bookingId, and a warning message —
                          the booking is NOT modified.
                        - Show the warning to the admin/trainer, then re-call this endpoint
                          with confirm=true to actually mark it Completed.
                        - Past or today-dated bookings complete immediately, no confirmation needed.
                      """
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Either the booking was completed, or confirmation is required (see confirmationRequired field)",
            content = @Content(schema = @Schema(implementation = com.gympro.booking.dto.BookingCompletionResponse.class))),
        @ApiResponse(responseCode = "403", description = "Access denied — role is not TRAINER or ADMIN"),
        @ApiResponse(responseCode = "404", description = "Booking not found")
    })
    @PostMapping("/complete/{id}")
    public com.gympro.booking.dto.BookingCompletionResponse complete(
            @PathVariable Long id,
            @RequestHeader("X-User-Role") String role,
            @Parameter(description = "Explicit confirmation to complete a future-dated booking")
        ...
        return service.completeBooking(id, confirm);
    }
