...
public class BookingAuditAspect {
    ...
    public void auditCompleted(JoinPoint joinPoint, Object result) {
        if (result instanceof com.gympro.booking.dto.BookingCompletionResponse response
                && response.isConfirmationRequired()) {
            auditLog.info("[AUDIT] BOOKING_COMPLETION_CONFIRMATION_REQUIRED | bookingId={}", joinPoint.getArgs()[0]);
            return;
        }
