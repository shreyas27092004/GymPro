...
class PaymentAuditAspectTest {
    ...
    @Test
    @DisplayName("auditRefundRequested logs paymentId, request and result")
    void auditRefundRequested_doesNotThrow() {
        when(joinPoint.getArgs()).thenReturn(new Object[]{1L, "requestDto"});

        assertDoesNotThrow(() -> auditAspect.auditRefundRequested(joinPoint, "pendingResult"));
    }

    @Test
    @DisplayName("auditRefundApproved logs paymentId, role and result")
    void auditRefundApproved_doesNotThrow() {
        when(joinPoint.getArgs()).thenReturn(new Object[]{1L, "ADMIN"});

        assertDoesNotThrow(() -> auditAspect.auditRefundApproved(joinPoint, "completedResult"));
    }

    @Test
    @DisplayName("auditRefundRejected logs paymentId, role and result")
    void auditRefundRejected_doesNotThrow() {
        when(joinPoint.getArgs()).thenReturn(new Object[]{1L, "ADMIN"});

        assertDoesNotThrow(() -> auditAspect.auditRefundRejected(joinPoint, "rejectedResult"));
    }
