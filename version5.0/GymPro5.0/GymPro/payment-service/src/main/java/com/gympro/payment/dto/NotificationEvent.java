...
public class NotificationEvent implements Serializable {
    ...
    // ── Payment fields ───────────────────────────────────────────────────
    private Long   paymentId;           // the Payment entity's own ID (refund workflow reference)
    ...
    /** Refund request submitted by a member → every admin */
    public static NotificationEvent refundRequested(String memberEmail, Double amount, String txnId,
                                                      Long paymentId, String reason) {
        NotificationEvent e = new NotificationEvent();
        e.eventType      = "REFUND_REQUESTED";
        e.recipientEmail = memberEmail; // admin emails aren't tracked here; in-app broadcast covers admins
        e.amount         = amount;
        e.txnId          = txnId;
    ...
    /** Refund request approved by admin → member */
    public static NotificationEvent refundApproved(String memberEmail, Long memberId, Double amount,
                                                     String txnId, Long paymentId) {
        NotificationEvent e = new NotificationEvent();
    ...
    /** Refund request rejected by admin → member */
    public static NotificationEvent refundRejected(String memberEmail, Long memberId, Double amount,
                                                     String txnId, Long paymentId, String adminNote) {
        NotificationEvent e = new NotificationEvent();
        e.eventType      = "REFUND_REJECTED";
        e.recipientEmail = memberEmail;
        e.userId         = memberId;
        e.userRole       = "MEMBER";
        e.amount         = amount;
        e.txnId          = txnId;
        e.paymentId      = paymentId;
        e.description    = adminNote;
        return e;
    }

    /** Refund fully processed → member (and admin broadcast) */
    public static NotificationEvent refundCompleted(String memberEmail, Long memberId, Double amount,
                                                      String txnId, Long paymentId) {
        NotificationEvent e = new NotificationEvent();
        e.eventType      = "REFUND_COMPLETED";
        e.recipientEmail = memberEmail;
        e.userId         = memberId;
        e.userRole       = "MEMBER";
        e.amount         = amount;
