...
import com.gympro.payment.dto.RefundDecisionDto;
import com.gympro.payment.dto.RefundRequestDto;
import com.gympro.payment.dto.SubscriptionRefundRequest;
...
public class PaymentController {
    ...
    // ── Internal: Subscription Cancellation Refund (called by plan-service) ────

    @Operation(
        summary     = "Record a subscription-cancellation refund (internal, called by plan-service)",
        description = "Step 5 of the enterprise cancellation flow: plan-service has already calculated the " +
                      "prorated (or zero) refund and deactivated the subscription; this records it against " +
                      "the original payment and notifies the member."
    )
    @PostMapping("/internal/subscription-refund")
    public ResponseEntity<Payment> subscriptionRefund(@RequestBody SubscriptionRefundRequest req) {
        log.info("Subscription cancellation refund request: subscriptionId={}, refundAmount=₹{}",
            req.getSubscriptionId(), req.getRefundAmount());
        return ResponseEntity.ok(
            service.refundForSubscriptionCancellation(req.getSubscriptionId(), req.getRefundAmount(), req.getReason())
        );
    }

    // ── Refund Workflow (member request → admin approval) ──────────────────────

    @Operation(
        summary     = "Request a refund (Member)",
        description = "Member submits a refund request for their own SUCCESS payment. "
                    + "Sets refundStatus=PENDING and notifies all admins. Role: MEMBER."
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Refund request submitted",
            content = @Content(schema = @Schema(implementation = Payment.class))),
        @ApiResponse(responseCode = "400", description = "Invalid request / payment not eligible"),
        @ApiResponse(responseCode = "403", description = "Payment does not belong to this member"),
        @ApiResponse(responseCode = "404", description = "Payment not found")
    })
    @PostMapping("/refund/request/{id}")
    public ResponseEntity<Payment> requestRefund(
            @Parameter(description = "Payment ID", example = "1") @PathVariable Long id,
            @RequestBody RefundRequestDto req) {
        log.info("Refund request: paymentId={}, memberId={}", id, req.getMemberId());
        return ResponseEntity.ok(service.requestRefund(id, req));
    }

    @Operation(
        summary     = "Approve a refund request (Admin)",
        description = "Approves a PENDING refund request and immediately processes it "
                    + "(APPROVED → PROCESSING → COMPLETED). Role: ADMIN."
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Refund approved and completed",
            content = @Content(schema = @Schema(implementation = Payment.class))),
        @ApiResponse(responseCode = "400", description = "No pending refund request for this payment"),
        @ApiResponse(responseCode = "403", description = "ADMIN role required"),
        @ApiResponse(responseCode = "404", description = "Payment not found")
    })
    @PostMapping("/refund/approve/{id}")
    public ResponseEntity<Payment> approveRefund(
            @Parameter(description = "Payment ID", example = "1") @PathVariable Long id,
            @Parameter(description = "Caller role – must be ADMIN", example = "ADMIN", required = true)
            @RequestHeader("X-User-Role") String role,
            @RequestBody(required = false) RefundDecisionDto body) {
        String note = body != null ? body.getNote() : null;
        return ResponseEntity.ok(service.approveRefund(id, role, note));
    }

    @Operation(
        summary     = "Reject a refund request (Admin)",
        description = "Rejects a PENDING refund request. Payment status is left untouched. Role: ADMIN."
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Refund rejected",
            content = @Content(schema = @Schema(implementation = Payment.class))),
        @ApiResponse(responseCode = "400", description = "No pending refund request, or missing reason"),
        @ApiResponse(responseCode = "403", description = "ADMIN role required"),
        @ApiResponse(responseCode = "404", description = "Payment not found")
    })
    @PostMapping("/refund/reject/{id}")
    public ResponseEntity<Payment> rejectRefund(
            @Parameter(description = "Payment ID", example = "1") @PathVariable Long id,
            @Parameter(description = "Caller role – must be ADMIN", example = "ADMIN", required = true)
            @RequestHeader("X-User-Role") String role,
            @RequestBody RefundDecisionDto body) {
        String note = body != null ? body.getNote() : null;
        return ResponseEntity.ok(service.rejectRefund(id, role, note));
    }

    @Operation(
        summary     = "List pending refund requests (Admin)",
        description = "Returns every payment currently awaiting refund approval (refundStatus=PENDING). Role: ADMIN."
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Pending refund requests",
            content = @Content(array = @ArraySchema(schema = @Schema(implementation = Payment.class)))),
        @ApiResponse(responseCode = "403", description = "ADMIN role required")
    })
    @GetMapping("/refund/pending")
    public ResponseEntity<List<Payment>> getPendingRefunds(
            @Parameter(description = "Caller role – must be ADMIN", example = "ADMIN", required = true)
            @RequestHeader("X-User-Role") String role) {
        if (!"ADMIN".equals(role)) throw new SecurityException("ADMIN only");
        return ResponseEntity.ok(service.getPendingRefunds());
    }
