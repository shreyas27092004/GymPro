...
public class LoggingAspect {
    ...
    @Pointcut("within(com.gympro.payment.controller..*)")
    public void controllerLayer() {}

    @Pointcut("within(com.gympro.payment.service..*)")
    ...
    public Object logMethodExecution(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object[] args     = maskSignature(joinPoint.getArgs());
    ...
    private Object[] maskSignature(Object[] args) {
        if (args == null || args.length == 0) return args;
        Object[] masked = Arrays.copyOf(args, args.length);
        for (int i = 0; i < masked.length; i++) {
            if (masked[i] != null && masked[i].toString().contains("razorpaySignature=")) {
                masked[i] = masked[i].toString().replaceAll("razorpaySignature=[^,)]*", "razorpaySignature=***");
            }
        }
        return masked;
    }
