...
public class EmailService {
    ...
    // Called when a refund is processed by ADMIN (direct refund, or the final
    // COMPLETED step of the request → approve workflow)
    ...
    // Sent to an ADMIN when a member submits a new refund request
    public void sendRefundRequestAdminEmail(String to, Double amount, String txnId, String reason) {
        String subject = "GymPro – New Refund Request 🔔";
        String body =
        	    "A member has requested a refund:\n\n" +
        	    "Amount      : ₹" + String.format("%.2f", amount) + "\n" +
        	    "Transaction : " + txnId + "\n" +
        	    "Reason      : " + reason + "\n\n" +
    ...
    // Sent to the MEMBER confirming their refund request was received
    public void sendRefundRequestReceivedEmail(String to, Double amount, String txnId) {
        String subject = "GymPro – Refund Request Received 📝";
        String body =
        	    "We've received your refund request!\n\n" +
        	    "Amount      : ₹" + String.format("%.2f", amount) + "\n" +
        	    "Transaction : " + txnId + "\n\n" +
        	    "Our team will review it shortly. You'll be notified as soon as a decision is made.\n" +
        	    "GymPro Team";
        sendEmail(to, subject, body);
    }

    // Sent to the MEMBER when their refund request is approved (processing begins)
    public void sendRefundApprovedEmail(String to, Double amount, String txnId) {
        String subject = "GymPro – Refund Approved ✅";
        String body =
        	    "Good news — your refund request has been approved!\n\n" +
        	    "Amount      : ₹" + String.format("%.2f", amount) + "\n" +
        	    "Transaction : " + txnId + "\n\n" +
        	    "We're now processing your refund.\n" +
        	    "GymPro Team";
        sendEmail(to, subject, body);
    }

    // Sent to the MEMBER when their refund request is rejected
    public void sendRefundRejectedEmail(String to, Double amount, String txnId, String reason) {
        String subject = "GymPro – Refund Request Rejected ❌";
        String body =
        	    "Your refund request could not be approved.\n\n" +
        	    "Amount      : ₹" + String.format("%.2f", amount) + "\n" +
        	    "Transaction : " + txnId + "\n" +
        	    "Reason      : " + reason + "\n\n" +
        	    "If you believe this is a mistake, please contact GymPro support.\n" +
        	    "GymPro Team";
        sendEmail(to, subject, body);
    }
