...
public class NotificationConsumer {
        ...
        try {
            switch (event.getEventType()) {
                ...
                case "REFUND_REQUESTED":
                    // Member gets a confirmation email that their request was received.
                    safeSendEmail("REFUND_REQUESTED", () -> emailService.sendRefundRequestReceivedEmail(
                        event.getRecipientEmail(), event.getAmount(),
                        nullSafe(event.getTxnId(), "N/A")));
                    // Every admin is notified in-app: "Refund Request Received"
                    broadcastToAdmins(event.getAdminUserIds(),
                        "Refund Request Received 🔔",
                        String.format("A refund of ₹%.2f was requested. Reason: %s",
                            event.getAmount(), nullSafe(event.getDescription(), "N/A")),
                        "PAYMENT", "ADMIN_REFUND_REQUESTED", event.getPaymentId());
                    break;

                case "REFUND_APPROVED":
                    safeSendEmail("REFUND_APPROVED", () -> emailService.sendRefundApprovedEmail(
                        event.getRecipientEmail(), event.getAmount(),
                        nullSafe(event.getTxnId(), "N/A")));
                    saveInApp(event.getUserId(),
                        nullSafe(event.getUserRole(), "MEMBER"),
                        "Refund Approved ✅",
                        String.format("Your refund of ₹%.2f was approved and is being processed. Txn: %s",
                            event.getAmount(), nullSafe(event.getTxnId(), "N/A")),
                        "PAYMENT", "REFUND_APPROVED", event.getPaymentId());
                    break;

                case "REFUND_REJECTED":
                    safeSendEmail("REFUND_REJECTED", () -> emailService.sendRefundRejectedEmail(
                        event.getRecipientEmail(), event.getAmount(),
                        nullSafe(event.getTxnId(), "N/A"), nullSafe(event.getDescription(), "Not specified")));
                    saveInApp(event.getUserId(),
                        nullSafe(event.getUserRole(), "MEMBER"),
                        "Refund Rejected ❌",
                        String.format("Your refund request of ₹%.2f was rejected. Reason: %s",
                            event.getAmount(), nullSafe(event.getDescription(), "Not specified")),
                        "PAYMENT", "REFUND_REJECTED", event.getPaymentId());
                    break;

                case "REFUND_COMPLETED":
                    safeSendEmail("REFUND_COMPLETED", () -> emailService.sendRefundEmail(
                        event.getRecipientEmail(), event.getAmount(),
                        nullSafe(event.getTxnId(), "N/A")));
                    saveInApp(event.getUserId(),
                        nullSafe(event.getUserRole(), "MEMBER"),
                        "Refund Completed 💰",
                        String.format("₹%.2f has been refunded. Txn: %s",
                            event.getAmount(), nullSafe(event.getTxnId(), "N/A")),
                        "PAYMENT", "REFUND_COMPLETED", event.getPaymentId());
                    broadcastToAdmins(event.getAdminUserIds(),
                        "Refund Completed 💰",
                        String.format("₹%.2f refund completed. Txn: %s",
                            event.getAmount(), nullSafe(event.getTxnId(), "N/A")),
                        "PAYMENT", "ADMIN_REFUND_COMPLETED", event.getPaymentId());
                    break;
    ...
    public void handleOtpEvent(@Payload NotificationEvent event) {
        ...
        try {
            ...
            } else if ("ADMIN_REGISTRATION_CODE".equals(event.getEventType())) {
                if (event.getOtp() == null || event.getOtp().isBlank()) {
                    throw new AmqpRejectAndDontRequeueException("OTP field is null or blank");
                }
                emailService.sendAdminRegistrationCodeEmail(
                    event.getRecipientEmail(),
                    nullSafe(event.getNewAdminName(), "A new user"),
                    nullSafe(event.getNewAdminEmail(), "unknown"),
                    event.getOtp());
                // Sent one-per-existing-admin — intentionally NOT stored as in-app (security)
            } else {
                throw new AmqpRejectAndDontRequeueException("Unknown OTP eventType: " + event.getEventType());
            }
            log.info("✅ [OtpQueue] Email sent to: {}", event.getRecipientEmail());
