...
class EmailServiceExtendedTest {
    ...
    @Test
    void testSendRefundRequestAdminEmail_Success() {
        setupMimeMock();
        assertDoesNotThrow(() ->
            emailService.sendRefundRequestAdminEmail(
                "admin@test.com", 500.0, "TXN-REFUND01", "Trainer never showed up")
        );
        verify(mailSender, times(1)).send(any(MimeMessage.class));
    }
    ...
    @Test
    void testSendRefundRequestReceivedEmail_Success() {
        setupMimeMock();
        assertDoesNotThrow(() ->
            emailService.sendRefundRequestReceivedEmail("member@test.com", 500.0, "TXN-REFUND01")
        );
        verify(mailSender, times(1)).send(any(MimeMessage.class));
    }

    @Test
    void testSendRefundRequestReceivedEmail_EmptyEmail_ThrowsException() {
        assertThrows(IllegalArgumentException.class, () ->
            emailService.sendRefundRequestReceivedEmail("", 100.0, "TXN-001")
        );
    }
    ...
    @Test
    void testSendRefundApprovedEmail_Success() {
        setupMimeMock();
        assertDoesNotThrow(() ->
            emailService.sendRefundApprovedEmail("member@test.com", 500.0, "TXN-REFUND01")
        );
        verify(mailSender, times(1)).send(any(MimeMessage.class));
    }
    ...
    @Test
    void testSendRefundRejectedEmail_Success() {
        setupMimeMock();
        assertDoesNotThrow(() ->
            emailService.sendRefundRejectedEmail(
                "member@test.com", 500.0, "TXN-REFUND01", "No evidence provided")
        );
        verify(mailSender, times(1)).send(any(MimeMessage.class));
    }

    @Test
    void testSendRefundRejectedEmail_EmptyEmail_ThrowsException() {
        assertThrows(IllegalArgumentException.class, () ->
            emailService.sendRefundRejectedEmail("", 100.0, "TXN-001", "reason")
        );
    }
