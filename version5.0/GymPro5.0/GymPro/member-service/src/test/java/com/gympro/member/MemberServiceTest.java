...
public class MemberServiceTest {
    ...
    @Test
    void createMember_ShouldThrowWhenPhoneAlreadyRegistered() {
        Member input = new Member();
        input.setEmail("jane@example.com");
        input.setName("Jane");
        input.setPhone("9876543210");

        when(memberRepository.findByPhone("9876543210")).thenReturn(Optional.of(sampleMember));
    ...
    @Test
    void createMember_ShouldSucceedWhenPhoneIsUnique() {
        Member input = new Member();
        input.setEmail("jane@example.com");
        input.setName("Jane");
        input.setPhone("7000000000");

        when(memberRepository.findByPhone("7000000000")).thenReturn(Optional.empty());
        when(memberRepository.save(any(Member.class))).thenAnswer(inv -> inv.getArgument(0));

        Member result = memberService.createMember(input);

        assertEquals("7000000000", result.getPhone());
        verify(memberRepository, times(1)).save(input);
    }
    ...
    @Test
    void updateMember_ShouldThrowWhenPhoneBelongsToAnotherMember() {
        Member anotherMember = new Member();
        anotherMember.setId(2L);
        anotherMember.setPhone("7000000000");

        Member updated = new Member();
        updated.setName("John Updated");
        updated.setPhone("7000000000");

        when(memberRepository.findById(1L)).thenReturn(Optional.of(sampleMember));
        when(memberRepository.findByPhone("7000000000")).thenReturn(Optional.of(anotherMember));

        DuplicatePhoneException ex = assertThrows(DuplicatePhoneException.class,
                () -> memberService.updateMember(1L, updated));

        assertTrue(ex.getMessage().contains("7000000000"));
        verify(memberRepository, never()).save(any());
    }

    @Test
    void updateMember_ShouldAllowKeepingOwnUnchangedPhoneNumber() {
        Member updated = new Member();
        updated.setName("John Updated");
        updated.setPhone("9876543210"); // same as sampleMember's own phone

        when(memberRepository.findById(1L)).thenReturn(Optional.of(sampleMember));
        // findByPhone resolves to the member's own record (itself), not a different one
        when(memberRepository.findByPhone("9876543210")).thenReturn(Optional.of(sampleMember));
        when(memberRepository.save(any(Member.class))).thenAnswer(inv -> inv.getArgument(0));
