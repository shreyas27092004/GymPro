...
export default function AdminTrainers() {
  ...
  const decide = async (trainer, action) => {
    setDecidingId(trainer.id);
    setError('');
    try {
      if (action === 'approve') await trainerApi.approve(trainer.id);
      else                       await trainerApi.reject(trainer.id);
      // Refresh both lists so the trainer moves out of "Pending" and
      // the main table reflects its new status.
      await load();
    } catch (e) {
      setError(e.response?.data?.message || `Failed to ${action} trainer`);
    } finally {
      setDecidingId(null);
    }
  };
      ...
      <div className="content-body">
        ...
        {pending.length > 0 && (
          <div className="card" style={{ marginBottom: 20, borderColor: 'var(--amber)' }}>
            <SectionHeader title={`⏳ ${pending.length} Trainer${pending.length === 1 ? '' : 's'} Awaiting Approval`} />
            <div className="table-wrap">
              <table>
                <thead>
                  <tr>
                    <th>Name</th><th>Email</th><th>Specialization</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {pending.map(t => (
...
function ScheduleModal({ trainer, onClose }) {
  ...
  const [form,    setForm]    = useState({
    trainerId: trainer.id, sessionDate: todayStr, startTime: '09:00', endTime: '10:00', maxCapacity: 1,
  });
  ...
  return (
    <Modal title={`${trainer.name}'s Schedule`} onClose={onClose} size="modal-lg">
      ...
      {showAdd && (
        ...
        >
          <FormGroup label="Date">
            <input type="date" min={todayStr} value={form.sessionDate} onChange={e => setForm({ ...form, sessionDate: e.target.value })} />
          </FormGroup>
      ...
      {loading ? <LoadingCenter /> : slots.length === 0 ? (
        <EmptyState icon={<Icon name="calendar" size={20} style={{ color: 'var(--text3)' }} />} text="No schedule slots yet" />
      ) : (
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Date</th><th>Time</th><th>Booked</th><th>Status</th></tr>
            </thead>
            <tbody>
              {slots.map(s => (
                <tr key={s.id}>
                  <td style={{ fontWeight: 600, color: 'var(--text)' }}>{s.sessionDate}</td>
                  <td>{s.startTime} – {s.endTime}</td>
                  <td>{s.bookedCount}/{s.maxCapacity}</td>
                  <td>
                    <span className={`badge ${s.cancelled ? 'badge-red' : (s.bookedCount >= s.maxCapacity ? 'badge-amber' : 'badge-green')}`}>
                      {s.cancelled ? 'CANCELLED' : (s.bookedCount >= s.maxCapacity ? 'FULL' : 'AVAILABLE')}
                    </span>
                  </td>
                </tr>
              ))}
