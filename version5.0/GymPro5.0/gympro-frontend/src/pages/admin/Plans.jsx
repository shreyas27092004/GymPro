            ...
            <div key={plan.id} className="card fade-in" style={{ position: 'relative', borderColor: plan.active ? 'var(--border)' : 'rgba(255,91,110,0.3)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <span style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600 }}>Tier {plan.priorityLevel ?? 0}</span>
...
function PlanForm({ initial, onClose, onSaved }) {
  const [form, setForm] = useState({
    ...
    trainerDiscountPercent: initial?.trainerDiscountPercent ?? 0,
    dedicatedTrainer: initial?.dedicatedTrainer ?? false,
    priorityBooking: initial?.priorityBooking ?? false,
    ...initial,
  });
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = {
        ...
        priorityLevel: parseInt(form.priorityLevel, 10) || 0,
        trainerDiscountPercent: parseFloat(form.trainerDiscountPercent) || 0,
      };
    ...
    <Modal title={initial ? 'Edit Plan' : 'Create Plan'} onClose={onClose}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="form-row">
          <FormGroup label="Priority Level">
            <input
              ...
              onChange={set('priorityLevel')}
              placeholder="0"
            />
          </FormGroup>
          <FormGroup label="Trainer Discount (%)">
            <input
              type="number" min="0" max="100" step="0.1"
              value={form.trainerDiscountPercent}
              onChange={set('trainerDiscountPercent')}
              placeholder="0"
            />
          </FormGroup>
        </div>
        <div style={{
          ...
          borderRadius: 8, padding: '8px 12px', fontSize: 12, color: 'var(--text3)',
        }}>
          Higher priority level = higher tier. Members can only upgrade to a plan with a <strong>higher</strong> priority level than their current one.
        </div>
        <div className="form-row">
          <FormGroup label="Dedicated Trainer">
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
              <input type="checkbox" checked={!!form.dedicatedTrainer} onChange={setCheck('dedicatedTrainer')} />
              Included in this plan
            </label>
          </FormGroup>
          <FormGroup label="Priority Booking">
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
              <input type="checkbox" checked={!!form.priorityBooking} onChange={setCheck('priorityBooking')} />
              Included in this plan
            </label>
          </FormGroup>
        </div>
