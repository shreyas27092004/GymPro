...
describe('MemberBookings — payment method dropdown', () => {
  ...
  const mockSlots = [
    { id: 101, sessionDate: '2099-01-05', startTime: '09:00', endTime: '10:00', maxCapacity: 5, bookedCount: 0 },
  ];
  ...
  async function reachPaymentStep() {
    ...
    await waitFor(() => screen.getByText(/Available Session/i));
  ...
  test('shows Payment Method dropdown with Cash and Online options', async () => {
  ...
  test('button label changes to Online when Online selected', async () => {
    const user = await reachPaymentStep();
    await user.selectOptions(screen.getByRole('combobox'), 'ONLINE');
    expect(screen.getByText('Pay ₹500 Online')).toBeInTheDocument();
  });
