...
describe('MemberPlans — payment modal', () => {
  ...
  test('both Cash and Online payment options present in dropdown', async () => {
    await openPaymentModal();
  ...
  test('button switches to Online label when Online selected', async () => {
    const user = await openPaymentModal();
    await user.selectOptions(screen.getByRole('combobox'), 'ONLINE');
    expect(screen.getByText('Pay ₹999 Online')).toBeInTheDocument();
  });
