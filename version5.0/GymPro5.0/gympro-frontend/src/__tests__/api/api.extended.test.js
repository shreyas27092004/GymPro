...
jest.mock('../../api/api', () => {
  ...
  return {
    ...
    planApi: {
      ...
      getUpgradeQuote:    (mId, newPlanId)      => mockApi.get(`/plans/upgrade-quote?memberId=${mId}&newPlanId=${newPlanId}`),
      upgrade:            (mId, mEmail, newPlanId) => mockApi.post(`/plans/upgrade?memberId=${mId}&memberEmail=${encodeURIComponent(mEmail)}&newPlanId=${newPlanId}`),
      getPrivileges:      (mId)                 => mockApi.get(`/plans/privileges/${mId}`),
    },
    paymentApi: {
...
describe('planApi — extended', () => {
  ...
  test('getUpgradeQuote calls GET /plans/upgrade-quote with memberId and newPlanId', async () => {
    await apiModule.planApi.getUpgradeQuote(1, 5);
    expect(apiModule._mockCall).toHaveBeenCalledWith('/plans/upgrade-quote?memberId=1&newPlanId=5');
  });

  test('upgrade calls POST /plans/upgrade with memberId, memberEmail, newPlanId', async () => {
    await apiModule.planApi.upgrade(1, 'user@gym.com', 5);
    expect(apiModule._mockCall).toHaveBeenCalledWith(
      `/plans/upgrade?memberId=1&memberEmail=${encodeURIComponent('user@gym.com')}&newPlanId=5`
    );
  });

  test('getPrivileges calls GET /plans/privileges/:memberId', async () => {
    await apiModule.planApi.getPrivileges(9);
    expect(apiModule._mockCall).toHaveBeenCalledWith('/plans/privileges/9');
  });
});
