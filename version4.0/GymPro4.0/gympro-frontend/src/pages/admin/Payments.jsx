    ...
    <div className="fade-in">
      ...
      <div className="content-body">
        ...
        <div className="stats-grid stats-grid-3" style={{ marginBottom: 24 }}>
          {[
            { label: 'Total Revenue', value: `₹${totalRevenue.toFixed(0)}`, color: 'var(--green)', icon: 'wallet' },
            { label: 'Pending Payments', value: pendingCount, color: 'var(--amber)', icon: 'clock' },
            { label: 'Total Refunds', value: `₹${refundTotal.toFixed(0)}`, color: 'var(--red)', icon: 'undo' },
          ].map(s => (
        ...
        <div className="card">
          ...
          {loading ? <LoadingCenter /> : filtered.length === 0 ? (
            <EmptyState icon={<Icon name="card" size={20} style={{ color: 'var(--text3)' }} />} text="No transactions found" />
          ) : (
