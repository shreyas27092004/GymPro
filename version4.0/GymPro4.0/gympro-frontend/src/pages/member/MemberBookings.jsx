    ...
    return (
      <Modal title="Book Training Session" onClose={onClose}>
        <form onSubmit={submitBooking} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <select
                ...
                onChange={e => {
                  ...
                  setForm(f => ({
                    ...
                    sessionDate: '', // must re-pick a date for the newly selected slot
                  }));
                }}
                required
              >
          ...
          {form.scheduleId && (
            <FormGroup label={`Choose a ${form.sessionDay} date`}>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {getUpcomingDatesForDay(form.sessionDay).map(iso => {
                  const active = form.sessionDate === iso;
                  return (
                    <button
                      key={iso}
                      type="button"
                      onClick={() => setForm(f => ({ ...f, sessionDate: iso }))}
              ...
              <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 6 }}>
                Only upcoming {form.sessionDay} dates are shown — past dates can't be selected.
              </div>
            </FormGroup>
          )}
