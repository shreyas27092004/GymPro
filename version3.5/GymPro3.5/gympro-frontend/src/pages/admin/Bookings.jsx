...
import { LoadingCenter, Alert, Modal, StatusBadge, EmptyState, SectionHeader, FormGroup } from '../../components/UI';
...
export default function AdminBookings() {
  ...
  const handleAction = async (id, action) => {
    try {
      ...
      if (action === 'complete') await bookingApi.complete(id);
      load();
    } catch (e) {
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          {b.status === 'CONFIRMED' && (
                            <>
                              ...
                              <button className="btn btn-danger btn-sm" onClick={() => handleAction(b.id, 'cancel')}>Cancel</button>
                            </>
                          )}
                        </div>
                      </td>
...
function NewBookingForm({ onClose, onSaved }) {
  ...
  const [form, setForm] = useState({ memberId: '', memberEmail: '', trainerId: '', trainerEmail: '', scheduleId: '', sessionDay: '', sessionTime: '', notes: '' });
  ...
  return (
    <Modal title="New Booking" onClose={onClose}>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {slots.length > 0 && (
          <FormGroup label="Available Slot">
            <select value={form.scheduleId} onChange={e => {
              const s = slots.find(x => x.id === +e.target.value);
              setForm(f => ({ ...f, scheduleId: s?.id || '', sessionDay: s?.dayOfWeek || '', sessionTime: `${s?.startTime} - ${s?.endTime}` }));
            }} required>
              <option value="">Select slot…</option>
              {slots.map(s => <option key={s.id} value={s.id}>{s.dayOfWeek}: {s.startTime} – {s.endTime}</option>)}
            </select>
          </FormGroup>
        )}
