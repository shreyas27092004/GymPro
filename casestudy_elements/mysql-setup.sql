-- ============================================================
-- GymPro MySQL Setup Script (v4.0 — updated)
-- Run this once before starting the services
-- Covers all 7 databases used by the 9 microservices
-- (gateway, eureka, and chatbot-service have no database)
-- ============================================================
...
-- ──────────────────────────────────────────────────────────
-- gympro_payment  (payment-service)  ← NEW
-- ──────────────────────────────────────────────────────────
USE gympro_payment;
...
CREATE TABLE IF NOT EXISTS payments (
    ...
    member_id       BIGINT,
    member_email    VARCHAR(100),
    booking_id      BIGINT,          -- set if paying for a session booking (nullable)
    subscription_id BIGINT,          -- set if paying for a plan subscription (nullable)
    amount          DOUBLE,
    payment_method  VARCHAR(30),     -- CREDIT_CARD | DEBIT_CARD | UPI | QR_CODE | CASH | RAZORPAY
    status          VARCHAR(20),     -- PENDING | SUCCESS | FAILED | REFUNDED | FREE
    transaction_id  VARCHAR(100),
    description     VARCHAR(255),
    paid_at         DATETIME,
    payment_type    VARCHAR(20) NOT NULL DEFAULT 'REGULAR'   -- REGULAR | FREE_SESSION
);

-- ──────────────────────────────────────────────────────────
-- gympro_notification  (notification-service)  ← NEW
-- ──────────────────────────────────────────────────────────
USE gympro_notification;

CREATE TABLE IF NOT EXISTS in_app_notifications (
    id             BIGINT AUTO_INCREMENT PRIMARY KEY,
    user_id        BIGINT       NOT NULL,   -- maps to member/trainer/admin ID in their own DB
    user_role      VARCHAR(20)  NOT NULL,   -- MEMBER | TRAINER | ADMIN
    title          VARCHAR(120) NOT NULL,
    message        VARCHAR(500) NOT NULL,
    type           VARCHAR(20)  NOT NULL,   -- BOOKING | PAYMENT | PLAN | SYSTEM
    event_type     VARCHAR(40)  NOT NULL,   -- e.g. BOOKING_CONFIRMED, PAYMENT_SUCCESS
    reference_id   BIGINT,                  -- id of the referenced booking/payment/etc, null for SYSTEM
    is_read         TINYINT(1)  NOT NULL DEFAULT 0,
    created_at     DATETIME     NOT NULL,
    deleted_at     DATETIME,                -- soft-delete timestamp, null = active

    CONSTRAINT uq_notification_dedup UNIQUE (user_id, event_type, reference_id)
);

CREATE INDEX idx_user_created ON in_app_notifications (user_id, created_at DESC);
CREATE INDEX idx_user_unread  ON in_app_notifications (user_id, is_read);
CREATE INDEX idx_user_role    ON in_app_notifications (user_id, user_role);

-- ============================================================
-- Setup complete! All 7 databases and tables created.
-- Note: gateway-service, eureka-server, and chatbot-service
