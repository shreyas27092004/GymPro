...
import { Alert, StatusBadge, EmptyState, SectionHeader, Modal, FormGroup, LoadingCenter } from '../../components/UI';
    ...
    <div className="content-body">
      ...
      <div className="card">
        ...
        {loading ? <LoadingCenter /> : filtered.length === 0 ? (
          <EmptyState icon="📅" text="No bookings yet. Book your first session!" />
        ) : (
...
function BookSessionModal({ member, onClose, onSaved }) {
  ...
  const [form,             setForm]             = useState({
    ...
    scheduleId:  '', sessionDay:   '', sessionTime: '', notes: '',
  });
  ...
  const onTrainerChange = async (e) => {
    ...
    setForm(f => ({ ...f, trainerId: t.id, trainerEmail: t.email, scheduleId: '', sessionDay: '', sessionTime: '' }));
            ...
            <div style={{
              ...
              {selectedTrainer.sessionFee != null ? (
                ...
                <span style={{ fontWeight: 600, color: 'var(--amber)', fontSize: 12 }}>
                  ⚠️ Trainer hasn't set a fee yet
                </span>
              )}
            </div>
          )}
    ...
    return (
      <Modal title="Session Booked!" onClose={onSaved}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="alert alert-success">✅ Session booked successfully!</div>
          <div style={{
            background: 'rgba(0,229,160,0.08)', border: '1px solid rgba(0,229,160,0.3)',
    ...
    <Modal title="Complete Payment" onClose={onSaved}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="alert alert-success">
          ✅ Session booked! Complete payment to confirm your spot.
        </div>
          ...
          >
            {loading
              ...
              : method === 'CASH'
                ...
                : `Pay ₹${sessionAmount} via Razorpay`
            }
          </button>
        </div>
      </div>
